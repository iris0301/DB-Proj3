...
public class FileList extends AbstractList <Comparable []> implements List <Comparable []>, RandomAccess,Serializable
{
    ...
    * @author Yunyun
    * @Description: TODO
    * @param  _domain  the string containing attribute domains   
    * @return int   the recordSize of a tuple 
     */
    /*
    private int computeRecordSize(Class [] _domain){    	
    	int recordSize = 0;
    	for(int i=0;i<_domain.length;i++){
        	if(_domain[i].equals(Long.class) ){
        		recordSize = recordSize + FileList.tupleLongLen;
        	}else if(_domain[i].equals(Integer.class) ){
        		recordSize = recordSize + FileList.tupleIntLen;
        	}else if(_domain[i].equals(Short.class) ){
        		recordSize = recordSize + FileList.tupleShortLen;
        	}else if(_domain[i].equals(Byte.class) ){
        		recordSize = recordSize + FileList.tupleBytetLen;
        	}else if(_domain[i].equals(Float.class)){
        		recordSize = recordSize + FileList.tupleFloatLen;
        	}else if(_domain[i].equals(Double.class)){
        		recordSize = recordSize + FileList.tupleDoubleLen;
        	}else if(_domain[i].equals(Character.class)){
        		recordSize = recordSize + FileList.tupleCharacterLen;
        	}else if(_domain[i].equals(String.class)){
        		recordSize = recordSize + FileList.tupleStringLen;
        	}
        }
    ...
    * @throws
    ...
    public byte[] pack(Comparable [] tuple){
    	...
    	for(int i = 0; i<tuple.length;i++){
    		...
    		try {
	    		 ...
	    		 * else if(domain[i].equals(Float.class) ){
	        		doutput.writeFloat(new Float(tuple[i]));
	        	}
	    		 */
    	...
    	/*
    	System.out.println("===========");
    	for(int i=0;i<attributeSize[nRecords].length;i++){
    		System.out.println(attributeSize[nRecords][i]);
    	}
    	*/
    	//System.out.println("==="+result.length);
    	return result;
    ...
    * @throws
    ...
    public  Comparable [] unpack(byte[] result,int index){
    	...
    	/*
    	//display
    	for(int i=0;i<tupleTemp.length;i++){
    		System.out.println(tupleTemp[i]);
    	}
    	*/
    	return tupleTemp;
    }
    ...
    public Comparable [] get (int index)
    {
    	...
    	//System.out.println(thisRecordSize);
